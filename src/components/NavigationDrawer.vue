  ...
  <v-navigation-drawer>
    <v-list class="pb-5">
      <v-list-item class="pb-2">
        <v-text-field id="group-search-input" :loading="false" class="d-sm-inline-block w-100"
          append-inner-icon="mdi-magnify" density="compact" label="Search" placeholder="Search" variant="outlined"
          hide-details single-line v-model="store.tagSearch"></v-text-field>
      </v-list-item>
          ...
          <v-menu :open-on-focus="false" activator="parent" open-delay="50" close-delay="50" open-on-hover submenu>
            <v-list class="py-0">
              <v-list-item link v-for="(endpoint, j) in globalStore.apiData.find(x => x.tag === tag)!.endpoints"
                ...
                <v-tooltip v-if="!isNullOrEmpty(endpoint.request.summary)" activator="parent" location="end">
